...
static BOOL SetPluginLoggingEnabledState(BOOL enabled, NSError **error) {
    ...
    if (enabled) {
        if (!DirectoryExistsAtPath(GetLogPath())) {
            if (!EnsureDirectoryAtURL(activeURL, error)) {
                return NO;
            }
        }
        // The directory is created via a root persona, but the plugin dylibs are
...
static BOOL ClearPluginLogsPreservingState(NSError **error) {
    ...
    // Re-create through the enable path so the directory keeps its world-writable
    // (sticky) permissions; a plain mkdir would revert it to root-only 0755.
    return SetPluginLoggingEnabledState(YES, error);
}
